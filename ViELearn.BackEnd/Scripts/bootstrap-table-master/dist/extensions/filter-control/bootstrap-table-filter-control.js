    ...
    var initFilterSelectControls = function (that) {
        ...
        $.each(that.header.fields, function (j, field) {
            ...
            if (isColumnSearchableViaSelect(column) && isFilterDataNotGiven(column) && hasSelectControlElement(selectControl)) {
                ...
                {
                    var jSelect = new Array();
                    jSelect = window[selector];                    
                    var uniqueValues = {};
                    if (jSelect != undefined) {
                        for (var i = 0; i < jSelect.length; i++) {
                            uniqueValues[jSelect[i].Text] = jSelect[i].Text;
                        }
                    }
        ...
        $.each(that.columns, function (i, column) {
            ...
            if (!column.filterControl) {
                ...
                if (column.searchable && that.options.filterTemplate[nameControl]) {                    
                    ...
                    isVisible = 'visible';
                    html.push(that.options.filterTemplate[nameControl](that, column.field, isVisible, column.placeholder, column.selector));
                }
            }
    ...
    $.extend($.fn.bootstrapTable.defaults, {
        ...
        filterTemplate: {
            input: function (that, field, isVisible, placeholder) {
                ...
                return sprintf('<input type="search" id="customBsSearch-%s" class="form-control bootstrap-table-filter-control-%s customBsSearch" style="width: 100%; visibility: %s" placeholder="%s">', field, field, isVisible, placeholder);
            },
            select: function (that, field, isVisible) {                
                return sprintf('<select id="customBsSearch-%s"  class="form-control bootstrap-table-filter-control-%s customBsSearch" style="width: 100%; visibility: %s" dir="%s"></select>',
                field,field, isVisible, getDirectionOfSelectOptions(that.options.alignmentSelectControlOptions));
            },
            datepicker: function (that, field, isVisible) {
                return sprintf('<input id="customBsSearch-%s"  type="text" class="form-control date-filter-control bootstrap-table-filter-control-%s customBsSearch" style="width: 100%; visibility: %s">', field, field, isVisible);
            }
        },
        //internal variables
        valuesFilterControl: []
